...
-- constants
local MIN_VERISON = 10
local SERVER_VERISON = "5.6.30-ngxLuaShard-0.0.1"
...
function _M.recv_handshake_response(conn)
    local packet, typ, err= _M.recv_packet(conn)
    if not packet then
        return false, err
    end
    if typ == "ERR" then
    ...
    local authLen =0
	authLen, pos = _M.get_byte3(packet, pos)
